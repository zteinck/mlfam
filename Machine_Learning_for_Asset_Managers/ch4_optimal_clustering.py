...
'''
def clusterKMeansBase(corr0, maxNumClusters=10, n_init=10, debug=False, verbose=False):
    corr0[corr0 > 1] = 1
    dist_matrix = ((1-corr0.fillna(0))/2.)**.5
    silh_coef_optimal = pd.Series(dtype='float64') #observations matrixs
    kmeans, stat = None, None
    maxNumClusters = min(maxNumClusters, int(np.floor(dist_matrix.shape[0]/2)))
    if verbose: print("maxNumClusters"+str(maxNumClusters))
...
'''
def clusterKMeansTop(corr0: pd.DataFrame, maxNumClusters=None, n_init=10, verbose=False):
    ...
    if len(redoClusters) <= 2:
        ...
        if verbose: print("clstrs2.len, stat:"+str(len(clstrs2.keys())))
        ...
        if newTstatMean <= tStatMean:
            if verbose: print("newTstatMean <= tStatMean"+str(newTstatMean)+ " (len:newClst)"+str(len(clstrsNew.keys()))+" <= "+str(tStatMean)+ " (len:Clst)"+str(len(clstrs.keys())))
            return corr1, clstrs, silh
        else:
            if verbose: print("newTstatMean > tStatMean"+str(newTstatMean)+ " (len:newClst)"+str(len(clstrsNew.keys()))
